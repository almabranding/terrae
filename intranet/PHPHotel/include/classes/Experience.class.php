 ...
 * 	                        
 * */
class Experience extends MicroGrid {
    ...
    private static $arr_type_vm = array(
        '0' => _NONE,
        '1' => 'Remember',
        '2' => 'Savor',
        '3' => 'Explore',
    ...
    function __construct() {
        ...
        if (isset($_POST['hotel_location_id']))
            $this->params['hotel_location_id'] = prepare_input($_POST['hotel_location_id']);
        if (isset($_POST['type']))
            $this->params['type'] = prepare_input($_POST['type']);
        if (isset($_POST['author_name']))
            $this->params['author_name'] = prepare_input($_POST['author_name']);
        if (isset($_POST['policies']))
            $this->params['policies'] = prepare_input($_POST['policies']);
        if (isset($_POST['phone']))
            $this->params['phone'] = prepare_input($_POST['phone']);
        if (isset($_POST['fax']))
            $this->params['fax'] = prepare_input($_POST['fax']);
        if (isset($_POST['category']))
            $this->params['category'] = prepare_input($_POST['category']);
        ...
        if (isset($_POST['time_zone']))
            $this->params['time_zone'] = prepare_input($_POST['time_zone']);
        ...
        if (isset($_POST['stars']))
            $this->params['stars'] = prepare_input($_POST['stars']);
        ...
        $this->dataSet = array();
        $this->error = '';
        $this->formActionURL = 'index.php?admin=experience_info';
        $this->actions = array(
        ...
        if ($this->hotelOwner) {
            ...
            if (!empty($hotels_list))
                $this->WHERE_CLAUSE .= ' AND ' . $this->tableName . '.' . $this->primaryKey . ' IN (' . $hotels_list . ')';
        }
        ...
        $arr_type = array('1' => 'Remember', '2' => 'Savor', '3' => 'Explore', '4' => 'Create');
        ...
        $this->arrTranslations = $this->PrepareTranslateFields(
                array('name', 'address', 'description', 'short_description')
        );
        ///////////////////////////////////////////////////////////////////////////////			
        ///////////////////////////////////////////////////////////////////////////////			
        ...
        $this->WHERE_CLAUSE.=' AND ' . $this->tableName . '.type=1 ';
        ...
        $this->arrViewModeFields = array(
            ...
            'category' => array('title' => _TYPE, 'type' => 'enum', 'align' => 'center', 'width' => '70px', 'sortable' => true, 'nowrap' => '', 'visible' => true, 'source' => self::$arr_type_vm),
        ...
        $this->arrAddModeFields = array(
            'separator_1' => array(
                ...
                'category' => array('title' => _TYPE, 'type' => 'enum', 'width' => '', 'required' => true, 'readonly' => false, 'default' => '', 'source' => $arr_type, 'default_option' => '', 'unique' => false, 'javascript_event' => '', 'view_type' => 'dropdownlist', 'multi_select' => false),
                ...
                'type' => array('title' => _IS_DEFAULT, 'type' => 'hidden', 'readonly' => false, 'default' => '1', 'true_value' => '1', 'false_value' => '0', 'unique' => false),
        ...
        $this->EDIT_MODE_SQL = 'SELECT
								...
								' . $this->tableName . '.room_picture_5_thumb,
                                                                ' . $this->tableName . '.author_picture,
                                                                ' . $this->tableName . '.author_picture_thumb
							FROM ' . $this->tableName . '
        ...
        $this->arrEditModeFields = array(
            ...
            'separator_2' => array(
                ...
                'author_picture' => array('title' => 'Author image', 'type' => 'image', 'width' => '210px', 'required' => false, 'target' => 'images/rooms_icons/', 'no_image' => '', 'random_name' => $random_name, 'image_name_pefix' => $image_prefix . 'author_', 'thumbnail_create' => true, 'thumbnail_field' => 'author_picture_thumb', 'thumbnail_width' => '190px', 'thumbnail_height' => '', 'file_maxsize' => '900k'),
        ...
        $this->arrDetailsModeFields = array(
            'separator_1' => array(
                ...
                'category' => array('title' => _TYPE, 'type' => 'enum', 'source' => self::$arr_type_vm),
    ...
    public static function GetAllActive($where_clause = '') {
        $sql = 'SELECT
					...
					' . TABLE_HOTELS_DESCRIPTION . '.short_description
				FROM ' . TABLE_HOTELS . '
				...
				WHERE
					' . TABLE_HOTELS . '.is_active = 1 AND type=1
    ...
    public static function HotelsCount() {
        $sql = 'SELECT COUNT(*) as cnt FROM ' . TABLE_HOTELS . ' WHERE is_active = 1 AND type=1';
        $result = database_query($sql, DATA_AND_ROWS, FIRST_ROW_ONLY);
        if ($result[1] > 0) {
    ...
    public static function DrawHotelDescription($hotel_id, $draw = true) {
        ...
        $sql = 'SELECT
				...
				WHERE h.is_active = 1 AND type=1 AND h.id = ' . (int) $hotel_id;
        ...
        if ($result[1] > 0) {
            ...
            if ($result[0]['phone'] || $result[0]['fax']) {
                if ($result[0]['phone'])
                    $output .= '<b>' . _PHONE . '</b>: ' . $result[0]['phone'] . '<br />';
                if ($result[0]['fax'])
                    $output .= '<b>' . _FAX . '</b>: ' . $result[0]['fax'];
            }
            $output .= '</td>';
            $output .= '</tr>';
            ...
            if ($result[0]['map_code'])
                $output .= '<p><b>' . _LOCATION . '</b>:<br /> ' . $result[0]['map_code'] . '</p>';
        }else {
        ...
        foreach ($this->arrTranslations as $key => $val) {
            $sql = 'UPDATE ' . TABLE_HOTELS_DESCRIPTION . '
					SET name = \'' . encode_text(prepare_input($val['name'])) . '\',
						...
						short_description = \'' . encode_text(prepare_input($val['short_description'], false, 'medium')) . '\'
    ...
    public function AfterDeleteRecord() {
        $sql = 'SELECT id, is_active FROM ' . TABLE_HOTELS . ' WHERE type=1';
