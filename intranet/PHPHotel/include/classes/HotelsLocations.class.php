...
class HotelsLocations extends MicroGrid {
	...
	{		
		...
		if(isset($_POST['section'])) $this->params['section'] = prepare_input($_POST['section']);
		...
		foreach($total_countries[0] as $key => $val){
			$arr_countries[$val['abbrv']] = $val['name'];
                ...
                $total_sections = $this->GetAllSections('priority_order DESC, name ASC');
		$arr_sections   = array();
		foreach($total_sections[0] as $key => $val){
			$arr_sections[$val['id']] = $val['name'];
		}
		...
		$this->arrAddModeFields = array(		    
			...
			'section' 	 => array('title'=>'Section', 'type'=>'enum',  'width'=>'210px', 'source'=>$arr_sections, 'required'=>true),
	...
	*/
	...
	public function AfterDeleteRecord()
	{
        ...
        public function GetAllSections(){
            $lang_id = Application::Get('lang');
		$sql = 'SELECT h.id,hd.name FROM home_sections h
                    JOIN home_sections_description hd ON h.id=hd.home_sections_id
