...
class RoomFacilities extends MicroGrid {
	...
	{		
		...
		$this->dataSet 		= array();
		$this->error 		= '';
		$this->formActionURL = ($type==0)?'index.php?admin=mod_room_facilities':'index.php?admin=mod_experience_facilities';
		...
		$this->VIEW_MODE_SQL = 'SELECT
								...
								FROM ('.$this->tableName.' 
									LEFT OUTER JOIN '.TABLE_ROOM_FACILITIES_DESCRIPTION.' rfd ON '.$this->tableName.'.id = rfd.room_facility_id AND rfd.language_id = \''.$this->languageId.'\' ) WHERE '.$this->tableName.'.type = \''.$this->type.'\'
								';		
	...
	{		
		$sql = 'SELECT
				...
				WHERE
					rf.is_active = 1 AND rf.type =0
				ORDER BY rf.priority_order ASC ';			
		return database_query($sql, DATA_AND_ROWS);
	}
        public static function GetAllActiveExperience()
	{		
		$sql = 'SELECT
					rf.*,
					rfd.name,
